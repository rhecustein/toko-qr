...
<html lang="{{session()->get('locale')}}" dir="{{$siteDirection}}">
                ...
                <div class="collapse navbar-collapse justify-content-between">
                                ...
                                <div class="header-profile">
                                    <div class="dropdown-menu dropdown-menu-right">
                                        @foreach(get_available_languages() as $lang)
                                            <a href="{{route('set.locale',['type'=>$lang])}}"
                                                ...
                                                <li class="notification-{{$notification->id}}">
                                                    <div class="timeline-panel">
                                                        ...
                                                        <div class="media-body">
                                                            ...
                                                            <small
                                                                class="d-block"
                                                                data-date="{{$notification->created_at}}">{{$notification->created_at->diffForHumans()}}</small>
                                                        </div>
                                                    </div>
                                                </li>
                                            @endforeach
                    ...
                    @else
                        @if(isset($rest_categories))
                            <ul class="navbar-nav header-right">
                                <li class="nav-item dropdown notification_dropdown">
                                    <a class="nav-link bell bell-link primary"
                                       href="{{route('registration',['type'=>'customer'])}}">
...
@php $site_setting=json_decode(get_settings('site_setting')) @endphp
@if(auth()->check() && isset($site_setting) && isset($site_setting->live_notification) && $site_setting->live_notification=='enable')
    <script src="{{asset("vendor/visibility/visibility.core.js")}}"></script>
    <script src="{{asset("vendor/visibility/visibility.timers.js")}}"></script>
    <script src="{{asset("js/notification-check.js")}}"></script>
@endif
